 ...
 */
static int fts_test_get_testparams(char *config_name, bool isHtc)
{
    int ret = 0;

    ret = fts_test_get_testparam_from_ini(config_name, isHtc);
...
static int fts_test_main_exit(int testType)
{
    int data_len = 0;
    FTS_TEST_FUNC_ENTER();
    ...
    if (testType) {
        fts_test_save_test_data( (testType == FOR_PANEL_TEST? HTC_TESTDATA_F_NAME_4_PANEL:HTC_TESTDATA_F_NAME_4_DEVICE), test_data.store_all_data, data_len);
        fts_test_save_test_data( (testType == FOR_PANEL_TEST? HTC_TESTRES_F_NAME_4_PANEL:HTC_TESTRES_F_NAME_4_DEVICE), test_data.testresult, test_data.testresult_len);
    } else {
 ...
 *
 */
static int fts_test_entry(char *ini_file_name, bool isHtc, int testType)
{
    int ret = 0;
    ...
    ret = fts_test_get_testparams(ini_file_name, isHtc);
    if (ret < 0) {
    ...
    if (true == fts_test_start()) {
        test_data.final_result = true;
        FTS_TEST_SAVE_INFO("\n=======Tp test pass. \n\n");
    } else {
        test_data.final_result = false;
        FTS_TEST_SAVE_INFO("\n=======Tp test failure. \n\n");
    }
...
#if HTC_FEATURE
void htc_test_ini_for_panel_device(int testType)
{
    struct fts_ts_data *ts_data = fts_data;
    struct input_dev *input_dev;
    ...
    input_dev = ts_data->input_dev;
    FTS_TEST_DBG("ini name:%s.", (testType == FOR_PANEL_TEST) ? INI_FOR_PANEL: INI_FOR_DEVICE);
    ...
    fts_test_entry( (testType == FOR_PANEL_TEST) ? INI_FOR_PANEL: INI_FOR_DEVICE, true, testType);
...
static ssize_t for_device_ini_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    int count = 0;
    test_data.final_result = false;

    FTS_TEST_FUNC_ENTER();
    htc_test_ini_for_panel_device(FOR_DEVICE_TEST);

    count += snprintf(buf + count, PAGE_SIZE, "%s\n", (test_data.final_result? "PASS":"FAIL"));

    FTS_TEST_FUNC_EXIT();
    return count;
}

static ssize_t for_panel_ini_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    int count = 0;
    test_data.final_result = false;

    FTS_TEST_FUNC_ENTER();
    htc_test_ini_for_panel_device(FOR_PANEL_TEST);
...
static DEVICE_ATTR(device_selftest, S_IRUGO | S_IWUSR, for_device_ini_show, NULL);
static DEVICE_ATTR(panel_selftest, S_IRUGO | S_IWUSR, for_panel_ini_show, NULL);
#endif
