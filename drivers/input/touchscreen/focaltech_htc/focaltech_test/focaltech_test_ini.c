...
int fts_test_get_ini_size(char *config_name, bool isHtc)
{
    ...
    if (isHtc) {
        sprintf(filepath, "%s%s", HTC_INI_FILE_PATH, config_name);
    } else {
...
int fts_test_read_ini_data(char *config_name, char *config_buf, bool isHtc)
{
    struct file *pfile = NULL;
    struct inode *inode = NULL;
    off_t fsize = 0;
 ...
 */
int fts_test_get_testparam_from_ini(char *config_name, bool isHtc)
{
    int ret = 0;
    ...
    inisize = fts_test_get_ini_size(config_name, isHtc);
    ...
    ret = fts_test_read_ini_data(config_name, ini_file_data, isHtc);
    if (ret) {
