...
enum enumTestItem_FT8607 {
    ...
    CODE_FT8607_INT_PIN_TEST = 27,
    CODE_FT8607_RST_PIN_TEST = 28,
};
...
static int ft8607_intPin_test(void);
static int ft8607_rstPin_test(void);
...
bool start_test_ft8607(void)
{
    ...
    for (item_count = 0; item_count < test_data.test_num; item_count++) {
        ...
        ///////////////////////////////////////////////////////FT8607_INT_PIN_TEST
        if (CODE_FT8607_INT_PIN_TEST == test_data.test_item[item_count].itemcode) {
            ret = ft8607_intPin_test();
            if (ERROR_CODE_OK != ret || (!temp_result)) {
                test_result = false;
                test_data.test_item[item_count].testresult = RESULT_NG;
            } else
        ...
        ///////////////////////////////////////////////////////FT8607_RST_PIN_TEST
        if (CODE_FT8607_RST_PIN_TEST == test_data.test_item[item_count].itemcode) {
            ret = ft8607_rstPin_test();
            if (ERROR_CODE_OK != ret || (!temp_result)) {
                test_result = false;
                test_data.test_item[item_count].testresult = RESULT_NG;
            } else
...
static int ft8607_intPin_test(void)
{
    int pinStatus1 = -1;
    int pinStatus2 = -1;
    FTS_TEST_INFO("\n\n==============================Test Item: -------- Interrupt Pin Test\n");
    enter_factory_mode();

    pinStatus1 = gpio_get_value(fts_data->pdata->irq_gpio);
    FTS_TEST_DBG("%s: pinStatus1 = %d", pinStatus1);
    write_reg(0x08, 0x01);

    pinStatus2 = gpio_get_value(fts_data->pdata->irq_gpio);
    FTS_TEST_DBG("%s: pinStatus2 = %d", pinStatus2);
    /* wait for int triggle, if has, test pass, otherwise fail */

    FTS_TEST_DBG("before pin=%d, after pin=%d", pinStatus1, pinStatus2);
    if(pinStatus1 != -1 && pinStatus2 != -1) {
        if(pinStatus1 != 1 || pinStatus2 != 0) {
            FTS_TEST_SAVE_INFO("\n==========INT Pin test is NG\n");
            return 1;
        } else {
            FTS_TEST_SAVE_INFO("\n==========INT Pin test is PASS\n");
...
void init_basicthreshold_ft8607(char *strIniFile)
{
    ...
    //////////////////////////////////////////////////////////// Interrupt pin test
    GetPrivateProfileString("Basic_Threshold", "int_pin_test", "0", str, strIniFile);
    ft8607_basic_thr.int_pin_test = fts_atoi(str);

    //////////////////////////////////////////////////////////// Reset pin test
    GetPrivateProfileString("Basic_Threshold", "rst_pin_test", "0", str, strIniFile);
    ft8607_basic_thr.rst_pin_test = fts_atoi(str);
...
void init_testitem_ft8607(char  *strIniFile)
{
    ...
    ft8607_item.INT_PIN_TEST = fts_atoi(str);

    /////////////////////////////////// RST_PIN_TEST
    GetPrivateProfileString("TestItem", "RST_PIN_TEST", "0", str, strIniFile);
    ft8607_item.RST_PIN_TEST = fts_atoi(str);
...
void set_testitem_sequence_ft8607(void)
{
    ...
    ////////////////////////////////////////////////// INT_PIN_TEST
    if ( ft8607_item.INT_PIN_TEST == 1)
    {
        fts_set_testitem(CODE_FT8607_INT_PIN_TEST);
    }

    ////////////////////////////////////////////////// RST_PIN_TEST
    if ( ft8607_item.RST_PIN_TEST == 1)
    {
        fts_set_testitem(CODE_FT8607_RST_PIN_TEST);
    }
