...
#define ETH0_DELAY_TIME 20000 /* in ms */
#define ETH0_RESET_TIME 100 /* in ms */
...
/* Do not suspend in seconds after probing
 */
static
void eth0_delay_suspend_work(struct work_struct *work) {
	struct r8152 *tp = container_of(work, struct r8152, eth0_delay_work.work);
	switch (tp->work_state) {
		case ETH0_WORK_ENQ_SUSPEND:
			if (rtl8152_carrier == 0) {
				usb_enable_autosuspend(tp->udev);
				pr_info("%s r8152: enable autosuspend\n",__func__);
				eth0_reset = true;
			}
			tp->work_state = ETH0_WORK_SUCCESS;
			wmb();
		break;
		case ETH0_WORK_ENQ_RESET:
			htc_reconnect_secondary_port();
			// Reset tp->work_state to ETH0_WORK_SUCCESS when doing disconnect function
		break;
		default:
		break;
	}
...
/* Handle delay work queue items.
 */
static
void htc_enqueue_delay_work(struct r8152 *tp, u8 work, u16 delay) {
	if (tp->work_state != work) {
		pr_info("%s r8152: Update work state %d -> %d\n",__func__, tp->work_state, work);
		tp->work_state = work;
		queue_delayed_work(tp->eth0_work_queue, &tp->eth0_delay_work, msecs_to_jiffies(delay));
	}
}

static
void htc_cancel_delay_work(struct r8152 *tp) {
	if (tp->work_state != ETH0_WORK_SUCCESS) {
		pr_info("%s r8152: Reset work state!!\n",__func__);
		cancel_delayed_work_sync(&tp->eth0_delay_work);
		tp->work_state = ETH0_WORK_SUCCESS;
	}
}
#endif
...
#if SUPPORT_WAN_PORT
		if (tp->hcd_core_id) {
			htc_enqueue_delay_work(tp, ETH0_WORK_ENQ_RESET, ETH0_RESET_TIME);
		}
#endif
	} else {
...
#if SUPPORT_WAN_PORT
			if (tp->hcd_core_id) {
				htc_enqueue_delay_work(tp, ETH0_WORK_ENQ_SUSPEND, ETH0_DELAY_TIME);
				rtl8152_carrier = 0;
			}
#endif
		}
